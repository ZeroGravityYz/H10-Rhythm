...
public final class MonitorService extends Service implements EcgEngine.Listener, EventStore.Listener {
    ...
    private static final String CH_MONITOR="monitor",CH_ALERT="alerts",CH_TECH="technical";private static final int ID_ONGOING=10;
    ...
    private BluetoothGattCharacteristic control,data;private int setupStep;private boolean scanning,running,streaming;private String status="Arrêté",deviceName="Polar H10";private int battery=-1;
    ...
    private void applyFeedback(Intent intent){float[] vector=intent.getFloatArrayExtra("morphology");String label=intent.getStringExtra("label");if("anomaly".equals(label))morphologyModel.confirmAnomaly(vector);else if("normal".equals(label))morphologyModel.confirmNormal(vector);persistModel();broadcast(true);}
    ...
    private void startMonitoring(){running=true;streaming=false;engine.reset();store.startSession();PowerManager pm=(PowerManager)getSystemService(POWER_SERVICE);wakeLock=pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,"PolarH10Monitor:ECG");wakeLock.setReferenceCounted(false);wakeLock.acquire(15*60_000L);status="Recherche de la H10…";broadcast(true);worker.post(this::connectKnownOrScan);worker.postDelayed(watchdog,10_000);}
    ...
    private final BluetoothGattCallback gattCallback=new BluetoothGattCallback(){
        ...
        @Override public void onCharacteristicWrite(BluetoothGatt g,BluetoothGattCharacteristic c,int statusCode){if(c.getUuid().equals(PMD_CONTROL)&&statusCode==BluetoothGatt.GATT_SUCCESS){worker.post(()->readBattery(g));}}
    ...
    private void continueSetup(BluetoothGatt g,int result){if(result!=BluetoothGatt.GATT_SUCCESS){technicalAlert("Configuration ECG échouée","Impossible d’activer les notifications PMD.");closeGatt();scheduleReconnect(3000);return;}if(setupStep==1){setupStep=2;enable(g,data,false);}else if(setupStep==2){setupStep=3;status="Commande de démarrage ECG…";writeCharacteristic(g,control,START_ECG);broadcast(true);}}
    ...
    private void writeCharacteristic(BluetoothGatt g,BluetoothGattCharacteristic c,byte[]v){if(Build.VERSION.SDK_INT>=33)g.writeCharacteristic(c,v,BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT);else{c.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT);c.setValue(v);g.writeCharacteristic(c);}}
    ...
    private void handleCharacteristic(UUID uuid,byte[]v){if(uuid.equals(PMD_DATA))worker.post(()->decodeEcg(v));else if(uuid.equals(PMD_CONTROL))worker.post(()->handleControl(v));}
    private void handleControl(byte[]v){if(v.length<4||(v[0]&255)!=0xf0)return;int op=v[1]&255,type=v[2]&255,result=v[3]&255;if(op==2&&type==0){if(result==0){streaming=true;status="Surveillance ECG active";lastData=SystemClock.elapsedRealtime();updateOngoing();broadcast(true);}else{technicalAlert("Démarrage ECG refusé","Réponse PMD "+result+". Ferme les autres applications Polar puis réessaie.");closeGatt();scheduleReconnect(5000);}}}

    private void decodeEcg(byte[]v){if(!running||v.length<13||(v[0]&255)!=0||(v[9]&255)!=0)return;int count=(v.length-10)/3;if(count<=0)return;long arrival=System.currentTimeMillis();for(int i=0;i<count;i++){int o=10+i*3,val=(v[o]&255)|((v[o+1]&255)<<8)|((v[o+2]&255)<<16);if((val&0x800000)!=0)val|=0xff000000;long t=arrival-Math.round((count-1-i)*1000.0/EcgEngine.FS);store.onRawSample(val,t);float f=engine.push(val,t);wave[wavePos]=f;wavePos=(wavePos+1)%wave.length;if(waveCount<wave.length)waveCount++;}lastData=SystemClock.elapsedRealtime();int learned=morphologyModel.normalCount();if(learned!=lastSavedModelCount&&(learned%10==0||learned==MorphologyModel.BASELINE_TARGET))persistModel();if(lastData-lastUi>500){lastUi=lastData;broadcast(true);updateOngoing();}}

    @Override public void onEvent(EcgEngine.DetectionEvent event){store.beginEvent(event);alertEvent(event);broadcast(true);}
    @Override public void onReportReady(String name,String location){main.post(()->{NotificationManager nm=(NotificationManager)getSystemService(NOTIFICATION_SERVICE);Notification n=baseBuilder(CH_TECH).setContentTitle("Rapport ECG prêt").setContentText(location).setSmallIcon(android.R.drawable.ic_menu_save).setAutoCancel(true).build();nm.notify((int)(System.currentTimeMillis()&0x7fffffff),n);broadcast(true);});}
    ...
    private void closeGatt(){if(gatt!=null){try{if(control!=null)writeCharacteristic(gatt,control,STOP_ECG);}catch(Exception ignored){}try{gatt.disconnect();gatt.close();}catch(Exception ignored){}gatt=null;control=data=null;}}
    private void stopMonitoring(){running=false;streaming=false;stopScan();worker.removeCallbacksAndMessages(null);closeGatt();store.stopSession();persistModel();if(wakeLock!=null&&wakeLock.isHeld())wakeLock.release();status="Surveillance arrêtée";broadcast(true);stopForeground(STOP_FOREGROUND_REMOVE);stopSelf();}
