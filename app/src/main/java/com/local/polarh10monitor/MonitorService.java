...
public final class MonitorService extends Service implements EcgEngine.Listener, EventStore.Listener {
    public static final String ACTION_START="com.local.polarh10monitor.START",ACTION_STOP="com.local.polarh10monitor.STOP",ACTION_UPDATE="com.local.polarh10monitor.UPDATE",
            ...
            ACTION_START_MORNING="com.local.polarh10monitor.START_MORNING",ACTION_CANCEL_MORNING="com.local.polarh10monitor.CANCEL_MORNING";
    ...
    private void startMorningCheck(Intent intent){
        ...
        morningActive=true;morningStartElapsed=SystemClock.elapsedRealtime();morningDataStartSample=engine.snapshot().samples+EcgEngine.FS*30L;morningGoodTicks=morningTotalTicks=0;morningMessage="30 secondes de stabilisation, puis mesure au calme.";
    ...
    private void startMonitoring(){running=true;streaming=accActive=false;battery=-1;lastBatteryAttempt=0;engine.reset();store.startSession();sessionStart=System.currentTimeMillis();sessionLastTrack=SystemClock.elapsedRealtime();sessionModerateMs=sessionVigorousMs=0;sessionBpmSum=sessionBpmCount=sessionMinBpm=sessionMaxBpm=0;morningActive=false;morningMessage="";configureTrainingZones();PowerManager pm=(PowerManager)getSystemService(POWER_SERVICE);wakeLock=pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,"PolarH10Monitor:ECG");wakeLock.setReferenceCounted(false);wakeLock.acquire(15*60_000L);status="Recherche de la H10…";broadcast(true);worker.post(this::connectKnownOrScan);worker.postDelayed(watchdog,10_000);}

    private void configureTrainingZones(){FitnessInsights.Profile profile=FitnessInsights.profile(this);if(!profile.complete()){sessionModerateThreshold=sessionVigorousThreshold=0;return;}FitnessInsights.Summary summary=FitnessInsights.summarize(this);double rest=summary.baselineHr>0?summary.baselineHr:70,max=208-.7*profile.age,reserve=Math.max(45,max-rest);sessionModerateThreshold=rest+.40*reserve;sessionVigorousThreshold=rest+.60*reserve;}
    ...
    private void handleRead(UUID uuid,byte[]v,int result){if(!uuid.equals(BATTERY_LEVEL))return;if(result==BluetoothGatt.GATT_SUCCESS&&v.length>0){battery=Math.max(0,Math.min(100,v[0]&255));if(battery<=10)technicalAlert("Batterie H10 faible",battery+" % restants. Remplace prochainement la pile.");broadcast(true);}else if(battery<0)scheduleBatteryRead(gatt,900,1);}
    ...
    private void trackSession(EcgEngine.Snapshot s){
        ...
        if(s.bpm>0){sessionBpmSum+=s.bpm;sessionBpmCount++;sessionMinBpm=sessionMinBpm==0?s.bpm:Math.min(sessionMinBpm,s.bpm);sessionMaxBpm=Math.max(sessionMaxBpm,s.bpm);}
    ...
    private void finishMorningCheck(){
        EcgEngine.FitnessMetrics metrics=engine.fitnessMetricsSince(morningDataStartSample);double timeQuality=morningTotalTicks==0?0:100.0*morningGoodTicks/morningTotalTicks,quality=Math.min(timeQuality,metrics.beatQualityPercent);morningActive=false;
        ...
        FitnessInsights.addMorning(this,new FitnessInsights.Morning(System.currentTimeMillis(),metrics.restingHr,metrics.rmssdMs,metrics.sdnnMs,quality,metrics.nnCount,morningSleep,morningStress,morningSoreness,morningSymptoms,morningAlcohol,morningHardTraining,morningExperimentId,morningExperimentFollowed));morningMessage="Bilan enregistré. Ton profil personnel a été actualisé.";broadcast(true);
    }
    private void scheduleReconnect(long delay){if(running)worker.postDelayed(this::connectKnownOrScan,delay);}
    private final Runnable watchdog=new Runnable(){@Override public void run(){if(!running)return;long now=SystemClock.elapsedRealtime();if(wakeLock!=null&&!wakeLock.isHeld())wakeLock.acquire(15*60_000L);if(streaming&&battery<0&&now-lastBatteryAttempt>15_000)scheduleBatteryRead(gatt,0,0);if(streaming&&lastData>0&&now-lastData>8_000){streaming=false;technicalAlert("Flux ECG interrompu","Aucun paquet ECG depuis 8 secondes. Reconnexion automatique.");closeGatt();scheduleReconnect(2_000);}worker.postDelayed(this,10_000);}};
    ...
    private void stopMonitoring(){EcgEngine.Snapshot finalSnapshot=engine.snapshot();running=false;streaming=false;morningActive=false;stopScan();worker.removeCallbacksAndMessages(null);closeGatt();store.stopSession();persistModel();int average=sessionBpmCount==0?0:Math.round(sessionBpmSum/(float)sessionBpmCount);SessionHistory.add(this,new SessionHistory.Record(sessionStart,System.currentTimeMillis(),average,sessionMinBpm,sessionMaxBpm,finalSnapshot.events,finalSnapshot.rmssdMs,finalSnapshot.sdnnMs,finalSnapshot.signalQualityPercent,sessionModerateMs,sessionVigorousMs));if(wakeLock!=null&&wakeLock.isHeld())wakeLock.release();status="Surveillance arrêtée";broadcast(true);stopForeground(STOP_FOREGROUND_REMOVE);stopSelf();}
