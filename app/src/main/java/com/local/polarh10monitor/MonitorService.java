...
public final class MonitorService extends Service implements EcgEngine.Listener, EventStore.Listener {
    ...
    private void startMonitoring(){running=true;streaming=accActive=false;battery=-1;lastBatteryAttempt=0;engine.reset();store.startSession();sessionStart=System.currentTimeMillis();sessionBpmSum=sessionBpmCount=sessionMinBpm=sessionMaxBpm=0;PowerManager pm=(PowerManager)getSystemService(POWER_SERVICE);wakeLock=pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,"PolarH10Monitor:ECG");wakeLock.setReferenceCounted(false);wakeLock.acquire(15*60_000L);status="Recherche de la H10…";broadcast(true);worker.post(this::connectKnownOrScan);worker.postDelayed(watchdog,10_000);}
